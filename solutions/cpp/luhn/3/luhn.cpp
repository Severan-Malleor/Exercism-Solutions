#include "luhn.h"
#include <string>
// Need this for stripping spaces from the string using the "erase-remove idiom"
#include <algorithm>
...
namespace luhn {
	...
	bool valid(std::string numbers) {
		...
		*/
		...
		// Standard C++ (C++11 to C++17) - static_cast is used to explicitly 
		// convert to a differnt type, like size_t to int.
		int total_length = static_cast<int>(numbers.length());
		...
		for (int i = total_length - 2; i >= 0; i -= 2) {
