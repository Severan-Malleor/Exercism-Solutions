...
// number of votes.
void increment_vote_count(ElectionResult& res, int vote_increase) {
    res.votes += vote_increase;
...
ElectionResult& determine_result(std::vector<ElectionResult>& results) {
    int top_vote_count = results[0].votes;
    int winner_index = 0;

    for (int i{ 1 }; i < results.size(); ++i) {
        if (results[i].votes > top_vote_count) {
            top_vote_count = results[i].votes;
            winner_index = i;
        }
    }
